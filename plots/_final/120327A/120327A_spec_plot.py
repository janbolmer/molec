...
= get_data("../../../spectra/GRB120327A_OB1UVB.txt",
	redshift, wl_range=True, wl1=w1, wl2=w2)

norm_spec = np.ones(len(wav_aa))*1.1

ignore_lst = []

synspec = SynSpec(wav_aa,redshift,9100, ignore_lst)

h2spec = synspec.add_ion(norm_spec,"HI",broad=25,Natom=22.0,A_REDSHIFT=0.0)
...
for axis in [ax1, ax2, ax3, ax4, ax5, ax6]:
	...
	axis.errorbar(wav_aa, n_flux/1.1, linestyle='-', color="black", linewidth=0.5, \
		drawstyle='steps-mid', label=r"$\sf Data$")

	axis.plot(wav_aa, h2spec/1.1, label=r"$\sf Fit$", color="#2171b5", linewidth=1.0, alpha=0.9)
