...
h2spec = synspec.add_CO(norm_spec,broad=15.31,NTOTCO=13.91,TEMP=3.993,A_REDSHIFT=-32.13/100000.0,NROT=[0, 1, 2, 3, 4, 5, 6])

wav_range = (max(wav_aa)-min(wav_aa))/2.0

fig = figure(figsize=(5, 10))

ax = fig.add_axes([0.17, 0.10, 0.81, 0.17*4+0.10])
...
ax1 = fig.add_axes([0.17, 0.10, 0.81, 0.17])
ax2 = fig.add_axes([0.17, 0.33, 0.81, 0.17])
ax3 = fig.add_axes([0.17, 0.56, 0.81, 0.17])
ax4 = fig.add_axes([0.17, 0.79, 0.81, 0.17])
...
for axis in [ax1, ax2, ax3, ax4]:
	...
	axis.plot(wav_aa, h2spec, label=r"$\sf Fit$", color="#2171b5", linewidth=1.8, alpha=0.9)
	...
	if axis == ax1:
		axis.set_xlim([1544.2*(1+redshift), 1545.5*(1+redshift)])
		for i in np.arange(0, len(co_name), 1):
			if (1544.2*(1+redshift)) < co_wav[i] < (1545.5*(1+redshift)):
				if i%2 == 0 and not i%3 == 0:
					...
					axis.text(co_wav[i]+0.05, 2.0, co_name[i], fontsize=8, color="#41ae76")
					axis.axvline(co_wav[i],  ymin=0.8, ymax=1.0, linestyle="-", color="#41ae76", linewidth=1.0)

	if axis == ax2:
		axis.set_xlim([1509.6*(1+redshift), 1510.1*(1+redshift)])
		for i in np.arange(0, len(co_name), 1):
	...
	if axis == ax3:
		axis.set_xlim([1477.0*(1+redshift), 1478.0*(1+redshift)])
		for i in np.arange(0, len(co_name), 1):
	...
	if axis == ax4:
		axis.set_xlim([1418.0*(1+redshift), 1420.0*(1+redshift)])
		for i in np.arange(0, len(co_name), 1):
			if (1418.0*(1+redshift)) < co_wav[i] < (1420.0*(1+redshift)):
				if i%2 == 0 and not i%3 == 0:
					axis.text(co_wav[i]+0.05, 1.6, co_name[i], fontsize=8, color="#41ae76")
					axis.axvline(co_wav[i], ymin=0.8, ymax=1.0, linestyle="-", color="#41ae76", linewidth=1.0)
				elif i%3 == 0 and not i%2 == 0:
...
#		if i%2 == 0:
#			ax3.text(h2_wav[i]+0.2, -0.5, h2_name[i], fontsize=10,color="#a50f15")
#			ax3.axvline(h2_wav[i], ymin=0.0, ymax=0.2, linestyle="-", color="#a50f15", linewidth=1.0)
#		else:
#			ax3.text(h2_wav[i]+0.2, -0.8, h2_name[i], fontsize=10,color="#a50f15")
#			ax3.axvline(h2_wav[i], ymin=0.0, ymax=0.2, linestyle="-", color="#a50f15", linewidth=1.0)
#	if (max(wav_aa)-wav_range*1) < h2_wav[i] < max(wav_aa):
#		if i%2 == 0:
#			ax2.text(h2_wav[i]+0.2, -0.5, h2_name[i], fontsize=10,color="#a50f15")
#			ax2.axvline(h2_wav[i], ymin=0.0, ymax=0.2, linestyle="-", color="#a50f15", linewidth=1.0)
#		else:
#			ax2.text(h2_wav[i]+0.2, -0.8, h2_name[i], fontsize=10,color="#a50f15")
#			ax2.axvline(h2_wav[i], ymin=0.0, ymax=0.2, linestyle="-", color="#a50f15", linewidth=1.0)
#
...
#ax3.axvline(1215.67*(2.2292+1), ymin=0.0, ymax=0.2, linestyle="-", color="blue", linewidth=2.0)
#
#ax3.text(1215.67*(2.2310+1)+0.2, -0.5, r"$\sf Ly\alpha$", fontsize=10,color="blue")
#ax3.text(1215.67*(2.2310+1)+0.2, -0.8, r"$\sf z=2.231$", fontsize=10,color="blue")
#
#
#
...
#
#ax3.text(3930.5, 1.9, r"$\sf I$",fontsize=11,color="#a50f15")
#ax3.text(3930.5, 1.63, r"$\sf II$",fontsize=11,color="#a50f15")
#ax3.text(3930.5, 1.36, r"$\sf III$",fontsize=11,color="#a50f15")
#
#lg = ax2.legend(numpoints=1, fontsize=10, loc=1)
#
#ax2.set_xlabel(r"$\sf Observed\, Wavelength (\AA)$", fontsize=24)
#ax.set_ylabel(r"$\sf Normalized\, Flux$", fontsize=24)
...
show()
fig.savefig(target + "CO_fit_spec_pub.pdf")
